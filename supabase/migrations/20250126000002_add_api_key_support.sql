-- Add API key support for brokers that don't use OAuth (e.g., Project X)
-- Make OAuth fields optional for API key-based connections

ALTER TABLE broker_connections
    ADD COLUMN IF NOT EXISTS api_key TEXT,
    ADD COLUMN IF NOT EXISTS api_secret TEXT,
    ADD COLUMN IF NOT EXISTS api_username TEXT, -- For Project X: username + API key (no secret)
    ADD COLUMN IF NOT EXISTS api_service_type TEXT, -- For Project X: 'topstepx' or 'alphaticks'
    ADD COLUMN IF NOT EXISTS api_base_url TEXT, -- Custom API base URL override
